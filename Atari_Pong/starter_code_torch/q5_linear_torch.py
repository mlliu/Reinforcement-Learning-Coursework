...
class Linear(DQN):
    ...
    def initialize_models(self):
        ...
        #print("linear layer size is: ",img_height*img_width*n_channels)
    ...
    def get_q_values(self, state, network='q_network'):
        ...
        Args:
            state: (torch tensor)
                shape = (batch_size, img height, img width, nchannels x config.state_history)  config.state_history=4
            network: (str)
        ...
        tensor = torch.flatten(state,start_dim=1) #end_dim faltten to(batch_size, num_pixes)
        #print("state's shape is: ",state.size())
        #print("input's shape is: ",tensor.size())
        if network == 'q_network':
            out = self.q_network(tensor)
        else:
            out = self.target_network(tensor)
        ##############################################################
        ######################## END YOUR CODE #######################
    ...
    def calc_loss(self, q_values : Tensor, target_q_values : Tensor,
        ...
        ##############################################################
        ##################### YOUR CODE HERE - 3-5 lines #############
        #Tensor.mask_fill(done_mask,0), where done_mask ==1, make the Tensor's value =0
        Q_samp = rewards +gamma*(torch.max(target_q_values,dim=1)[0].masked_fill(done_mask,0)) #element-wise product between done_mask and 
        #print("Q_samp is ",Q_samp)
        #print("the size of actions is ",actions.size())
        act_mat = F.one_hot(actions.long(),num_classes=num_actions)# an one hot vector
        Q = torch.sum(q_values*act_mat,dim=1) #again, element-wise product to choose the Q(s,a)
        #print("Q is ",Q)
        loss = F.mse_loss(Q_samp,Q) #sum of all loss
        return loss 
        ##############################################################
        ######################## END YOUR CODE #######################
